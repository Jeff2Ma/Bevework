...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
        err( __('错误：请填写必填区域 (昵称、邮箱)')); // 將 wp_die 改為錯誤提示
	elseif ( !is_email($comment_author_email))
        err(__('错误：电子邮箱地址无效，请检查')); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
    err(__('错误：评论框没有内容')); // 將 wp_die 改為錯誤提示
...
if ( $edit_id ) {  
  if ( get_comment_author_email($edit_id) != $comment_author_email ) err("您不能编辑该条评论！");  
